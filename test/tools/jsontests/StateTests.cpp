...
json_spirit::mValue doStateTests(json_spirit::mValue const& _v_orig, bool _fillin)
{
	BOOST_REQUIRE_MESSAGE(!_fillin || _v_orig.get_obj().size() == 1,
		TestOutputHelper::testFileName() + " A GeneralStateTest filler should contain only one test.");
	json_spirit::mValue v = _v_orig;  // TODO: avoid copying and only add valid fields into the new object.
